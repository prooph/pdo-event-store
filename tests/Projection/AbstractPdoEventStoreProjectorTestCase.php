...
use ProophTest\EventStore\Projection\AbstractEventStoreProjectorTestCase;
use Prophecy\PhpUnit\ProphecyTrait;
use ReflectionClass;
use ReflectionMethod;

abstract class AbstractPdoEventStoreProjectorTestCase extends AbstractEventStoreProjectorTestCase
{
    use ProphecyTrait;
    ...
    public function it_should_create_projection_stream_on_emit(): void
    {
        ...
        $projection
            ->fromStream('user-123')
            ->when([
                UserCreated::class => function (array $state, Message $event): void {
                    $this->emit($event);
                },
            ])
            ->run(false);
